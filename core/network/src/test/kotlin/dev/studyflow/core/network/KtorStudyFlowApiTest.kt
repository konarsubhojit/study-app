...
class KtorStudyFlowApiTest {
    @Test
    fun `a documented response decodes into typed models`() = runTest {
    ...
    @Test
    fun `the query parameter documented for tasks is sent`() = runTest {
        var requestedUrl = ""
        val api = MockBackend.api { request ->
    ...
    @Test
    fun `every request carries the client version so the server can police its population`() = runTest {
    ...
    @Test
    fun `an expired access token is refreshed and the call succeeds without the caller noticing`() = runTest {
    ...
    @Test
    fun `a spent refresh token signs the user out instead of looping`() = runTest {
        val store = InMemoryTokenStore(AuthTokens("expired", "refresh"))
        val api = MockBackend.api(tokenStore = store, tokenRefresher = { null }) {
            json("""{"code":"token_expired","message":"expired"}""", HttpStatusCode.Unauthorized)
        }
    ...
    @Test
    fun `a failing server is retried, and the final failure is a user-facing error`() = runTest {
    ...
    @Test
    fun `a write is never replayed, because a duplicate session is worse than a failure`() = runTest {
        val attempts = AtomicInteger()
        val api = MockBackend.api {
            attempts.incrementAndGet()
            json("""{"code":"upstream_unavailable","message":"boom"}""", HttpStatusCode.ServiceUnavailable)
    ...
    @Test
    fun `a conflict is explained rather than retried`() = runTest {
        val api = MockBackend.api {
            json("""{"code":"session_exists","message":"already stored"}""", HttpStatusCode.Conflict)
        }

        val error = api.uploadSession(
    ...
    @Test
    fun `a server that refuses this build asks the user to upgrade`() = runTest {
        val api = MockBackend.api {
    ...
    @Test
    fun `the minimum-client header cuts this build off before the server starts rejecting it`() = runTest {
        val api = MockBackend.api {
    ...
    @Test
    fun `a supported build is not disturbed by the minimum-client header`() = runTest {
        val api = MockBackend.api {
    ...
    @Test
    fun `a rate-limited call reports the wait the server asked for`() = runTest {
        val api = MockBackend.api {
            json("""{"code":"rate_limited","message":"slow down"}""", HttpStatusCode.TooManyRequests, retryAfter = "30")
        }
    ...
    @Test
    fun `an unreachable server reads as offline, not as a crash`() = runTest {
    ...
    @Test
    fun `a response missing a required field is a contract breach, not a crash`() = runTest {
    ...
    @Test
    fun `an error body the server did not send still produces a user-facing message`() = runTest {
        val api = MockBackend.api { json("<html>502 Bad Gateway</html>", HttpStatusCode.BadGateway) }
