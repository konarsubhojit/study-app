 ...
 */
public enum class UserFacingMessage(
...
public sealed class ApiError(
    ...
    public class Timeout(
        cause: Throwable? = null,
    ) : ApiError(UserFacingMessage.Slow, cause = cause)

    /** Credentials are missing, expired beyond refresh, or rejected. */
    public class Unauthorized(
        code: String? = null,
    ) : ApiError(UserFacingMessage.SignInRequired, code)

    /** Authenticated, but not permitted. */
    public class Forbidden(
        code: String? = null,
    ) : ApiError(UserFacingMessage.NotAllowed, code)

    /** The resource does not exist, or no longer does. */
    public class NotFound(
        code: String? = null,
    ) : ApiError(UserFacingMessage.Missing, code)

    /** The server's copy has moved on since the client's. */
    public class Conflict(
        code: String? = null,
    ) : ApiError(UserFacingMessage.Conflict, code)
    ...
    public class Server(
        code: String? = null,
    ) : ApiError(UserFacingMessage.ServerProblem, code)
    ...
    public class Malformed(
        cause: Throwable? = null,
    ) : ApiError(UserFacingMessage.Unexpected, cause = cause)
